...
export default function MenusAdminPage() {
    ...
    const handleDeleteMenu = async () => {
        ...
        if (!(await toast.confirm(`Are you sure you want to delete menu "${selectedMenu.name}"? This cannot be undone.`))) return;
    ...
    const handleActivateMenu = async (menu) => {
        if (!(await toast.confirm(`Are you sure you want to set "${menu.name}" as the ACTIVE public menu?`))) return;
        try {
            setActivating(true);
    ...
    const handleDeactivateMenu = async (menu) => {
        if (!(await toast.confirm(`Are you sure you want to DEACTIVATE "${menu.name}"? The site will revert to default navigation.`))) return;
        try {
