...
import { X, Star, ArrowRight, ShoppingBag, Heart } from 'lucide-react';
import { Button } from '@/components/ui';
import { useCart } from '@/context/CartContext';

export default function CartPage() {
    const { cartItems, removeFromCart, moveToWishlist, isInWishlist } = useCart();
                                                ...
                                                <div className="flex flex-col md:grid md:grid-cols-[3fr_1fr_1fr] md:items-center p-4 md:p-0 min-h-[120px]">
                                                    ...
                                                    <div className="flex items-center justify-between md:justify-start md:h-full mt-2 md:mt-0">
                                                        ...
                                                        <button
                                                            ...
                                                            className={`flex md:hidden items-center gap-2 px-4 py-2 rounded-lg transition-colors ${alreadyInWishlist
                                                                ? 'bg-[#F0F2F5] text-[#153A8D] opacity-80 cursor-default'
                                                                : 'bg-[#F0F2F5] text-[#153A8D] hover:bg-[#E3E8F2]'
                                                            }`}
                                                        >
                                                            <Heart size={14} className="stroke-[2.5px]" />
                                                            <span className="text-[11px] font-bold uppercase tracking-wider">
                                                                {alreadyInWishlist ? 'Wishlisted' : 'Add To Wishlist'}
                                                            </span>
                                                        </button>
                                                        ...
                                                        <button
                                                            onClick={() => moveToWishlist(item.id)}
                                                            disabled={alreadyInWishlist}
                                                            className={`hidden md:block text-sm font-bold py-2.5 whitespace-nowrap uppercase tracking-tight transition-colors ${alreadyInWishlist
                                                                ? 'text-[#153A8D] opacity-80 cursor-default'
                                                                : 'text-[#153A8D] hover:bg-[#153A8D]/10'
                                                            }`}
                                                        >
